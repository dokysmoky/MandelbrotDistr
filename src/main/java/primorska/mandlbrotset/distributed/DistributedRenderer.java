...
public class DistributedRenderer {
    ...
    /**
     * Distributed benchmark method without graphics.
     * Runs Mandelbrot calculation and MPI communication only.
     * Returns elapsed time in milliseconds (only meaningful on rank 0).
    ...
    public static long benchmarkDistributed(int width, int height,
        ...
        // Mandelbrot calculation (same as render)
        for (int py = startY; py < endY; py++) {
            for (int px = 0; px < width; px++) {
                double x0 = minX + px * (maxX - minX) / width;
                double y0 = minY + py * (maxY - minY) / height;
        ...
        // Prepare for gathering pixel data on rank 0
        int[] fullPixels = null;
        int[] recvCounts = new int[size];
        int[] displs = new int[size];
